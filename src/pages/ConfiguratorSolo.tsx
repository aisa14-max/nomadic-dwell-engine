...
export default function ConfiguratorSolo() {
  ...
  // replacement, not a small patch — with no rib/membrane-colored versions
  // of it, it would otherwise sit on top of _dwellingImg and hide any
  // custom colour picked above. Only show it for the default rib/membrane combo.
  const _showZoneOverlay = zoneAdded && _dwellingImg === dwellingFg;
    ...
    setMessages((prev) => {
      ...
      next[next.length - 1] = {
        ...
        content: addsWindows
          ...
          : makesShorter
          ? "Shortened the dwelling — take a look. Do you like this change?"
  ...
  const handleShorterChoice = async (liked: boolean) => {
    ...
    setMessages((prev) => {
      ...
      next[next.length - 1] = {
        role: "assistant",
        content: liked
          ? "Great — keeping the shorter dwelling."
          : "No problem — reverted back to the original length.",
      };
      return next;
    });
    setIsStreaming(false);
                        ...
                        >
                          {zones.map((z) => (
                            <ZoneCard
                              ...
                              glow={glowHintsEnabled && z.id === "kitchen" && !zoneAdded}
                            />
                          ))}
                        </div>
                        <button
